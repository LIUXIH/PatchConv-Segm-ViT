...
def scipy_misc_imresize(arr, size, interp='bilinear', mode=None):
    ...
    if np.issubdtype(ts, np.signedinteger):
        precent = size / 100.0
        size = tuple((np.array(im.size) * precent).astype(int))
...
class Resize(object):
    ...
    def __call__(self, img):
        ...
        """
        if img.ndim == 3:
            return scipy_misc_imresize(img, self.size, self.interpolation)
        elif img.ndim == 2:
            return scipy_misc_imresize(img, self.size, self.interpolation, 'F')
        else:
